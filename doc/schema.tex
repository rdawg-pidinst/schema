...
\item \emph{The Research Data Alliance Persistent Identification of Instruments
  ...
  ({\small \url{https://www.rd-alliance.org/groups/persistent-identification-instruments-wg/members/}})
\end{itemize}
...
This section documents changes to the schema with respect to previous
versions.

\subsection{Updates in Version 1.1}

Version 1.1 of the PIDINST schema includes these changes:
\begin{itemize}
\item Add new property MeasurementTechnique with sub-properties.
\item Add new property MeasuredQuantity with sub-properties.
\item Add new property Subject with sub-properties.
\item Add SWHID to the controlled list of values for
  relatedIdentifierType.
\item Deprecate MeasuredVariable in favor of MeasuredQuantity.
\end{itemize}

In addition to these schema changes, starting with version 1.1, we now
provide ``official'' definitions for the serialization of the schema
in LinkML and XML.
...
\item Optional (O): the property may be added to provide a richer
  description, or
\item Deprecated (D): the property is deprecated: another property has
  been added to the schema to replace it in the future.
\end{itemize}
...
\begin{longtable}{|l|l|l|}
  ...
  8     & Description          & R \\
  9     & InstrumentType       & R \\
  10    & MeasuredVariable     & D \\
  11    & Date                 & R \\
  12    & RelatedIdentifier    & R \\
  13    & AlternateIdentifier  & R \\
  14    & MeasurementTechnique & R \\
  15    & MeasuredQuantity     & R \\
  16    & Subject              & O \\
  \hline
...
\newlength{\idcolw}\settowidth{\idcolw}{14.2.1}
...
\begin{longtable}{|l|p{\propcolw}|l|p{\defcolw}|p{\valcolw}|}
  \hline
  ...
  5      & Owner                                & 1--n
         & Institution(s) responsible for the management of the
           instrument instance.  This may include the legal owner, the
           operator, or an institute providing access to the instrument.
         & \\
  \hline
  5.1    & ownerName                            & 1
  ...
  6      & Manufacturer                         & 1--n
         & The instrument's manufacturer(s) or developer.  This may
           also be the owner for custom-built instruments.
         & \\
  \hline
  ...
  \hline
  7      & Model                                & 0--1
         & Name of the model or type of instrument as attributed by
           the manufacturer
         & \\
  \hline
  7.1    & modelName                            & 1
  ...
  \hline
  8      & Description                          & 0--1
         & Technical description of the instrument instance and its
           capabilities
         & Free text \\
  \hline
  ...
  10     & MeasuredVariable                     & 0--n
         ...
         & \begin{minipage}[t]{\valcolw}
             Free text \\
             \emph{Deprecated}, please use MeasuredQuantity
           \end{minipage} \\
  \hline
  11     & Date                                 & 0--n
         & Dates relevant to the instrument instance & ISO 8601 \\
  \hline
  11.1   & dateType                             & 1
  ...
  13.2   & alternateIdentifierName              & 0--1
         ...
         & Free text \\
  \hline
  14     & MeasurementTechnique                 & 0--n
         & The protocol that the instrument follows or the physical
           phenomenon it makes use of in order to make its observation
         & \\
  \hline
  14.1   & measurementTechniqueName             & 1
         & Full name of the measurement technique  & Free text \\
  \hline
  14.2   & measurementTechnique\-Identifier     & 0--1
         & Identifier used to identify the measurement technique
         & Free text, should be a globally unique identifier \\
  \hline
  14.2.1 & measurementTechnique\-IdentifierType & 1
         & Type of the identifier & Free text \\
  \hline
  15     & MeasuredQuantity                     & 0--n
         & The variable(s) that this instrument measures or observes
         & \\
  \hline
  15.1   & measuredQuantityName                 & 1
         & Full name of the variable & Free text \\
  \hline
  15.2   & measuredQuantityIdentifier           & 0--1
         & Identifier used to identify the variable
         & Free text, should be a globally unique identifier \\
  \hline
  15.2.1 & measuredQuantity\-IdentifierType     & 1
         & Type of the identifier & Free text \\
  \hline
  16     & Subject                              & 0--n
         & Subject or keyword describing the instrument & \\
  \hline
  16.1   & subjectName                          & 1
         & Subject name or text & Free text \\
  \hline
  16.2   & subjectIdentifier                    & 0--1
         & Identifier in the case it refers to a term from an external
           vocabulary
         & Free text, should be a globally unique identifier \\
  \hline
  16.2.1 & subjectIdentifierType                & 1
         & Type of the identifier & Free text \\
  \hline
...
instrumentTypeName.  If the classification further defines identifiers
for the terms, it is recommended to link them using
...
modelIdentifierType, instrumentTypeIdentifierType,
measurementTechniqueIdentifierType, measuredQuantityIdentifierType,
and subjectIdentifierType we anticipate that there might be other,
less well established identifier types that may need to be used in
some cases.  Hence we allow free text.  Nevertheless, the values
defined for relatedIdentifierType should also be used here, whenever
it is possible.
...
Many instruments are designed to record one or maybe a few observable
properties.  In these cases it is recommended to add a
MeasuredQuantity for each of these observables.  Other instruments are
implementing a more complex measurement protocol in order to make an
observation.  A list of observables might not be suitable to describe
that.  In these cases it is recommended to reference that protocol as
a MeasurementTechnique instead.  In some cases it might even be
possible to provide both: MeasuredQuantity to describe what the
instrument observes and MeasurementTechnique to describe how that
observation is done.

The MeasuredVariable property from PIDINST version 1.0 has been
deprecated in favour of MeasuredQuantity, because it is a simple
property that does include an identifier sub-property and thus does
not allow linking terms from external vocabularies.  Please use
MeasuredQuantity when creating a PIDINST record.  If you are concerned
about compatibility with older consumers of the record, you may
additionally add measuredQuantityName as a MeasuredVariable.
Consumers of a PIDINST record should always check MeasuredQuantity
first and only consider MeasuredVariable as a fallback when the former
is not provided.
...
\begin{longtable}{|l|p{\valdefcolw}|}
  ...
  \\ \hline
  RAiD    &
  Research Activity Identifier, see \url{https://www.raid.org/}
  \\ \hline
  RRID    &
...
\newblock Metadata Schema for the Persistent Identification of Scientific
  Instruments.
...
\newblock DataCite Metadata Schema Documentation for the Publication
  and Citation of Research Data and Other Research Outputs, Version
  4.7, 2026.
\newblock \url{https://doi.org/10.14454/qdd3-ps68}.
